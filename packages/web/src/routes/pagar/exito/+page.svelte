...
<div class="max-w-md mx-auto pt-8">
  {#if data.estado === 'Pagado'}
    <h1 class="text-2xl font-bold text-emerald-700 mb-2">{t('exito.titulo')}</h1>
    <p class="text-gray-800 mb-1">{t('exito.descripcion')}</p>
    <p class="text-gray-700 mb-4">{t('exito.preventa')}</p>
  ...
  <div class="mt-3 text-sm text-gray-600 space-y-1">
    {#if data.pedidoId}<div>{t('exito.numero_pedido')} <code>{data.pedidoId}</code></div>{/if}
