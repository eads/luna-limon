// sst.config.ts  ── SST v3-style (no eager imports)
import type { SSTConfig } from "sst";           // ✅ type-only, erased
...
const config: SSTConfig = {
  ...
  // 2 · load and register the stacks -----------------------------------
  async stacks(app) {
    // dynamic imports keep the platform happy
    const { ServicesStack } = await import("./stacks/services");
    const { SiteStack      } = await import("./stacks/site");

    app.stack(ServicesStack, { id: "services" });
    app.stack(SiteStack,     { id: "site"      });
  },
};
